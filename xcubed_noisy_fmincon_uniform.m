...
rt=rnall(find(M==Mval));
% deterministic initializations
eps=0.01;
xsamp=linspace(a+eps,b-eps,rt);
xminit=nchoosek(xsamp,M-1);
xminit=[ones(size(xminit,1),1)*a xminit ones(size(xminit,1),1)*b];
rn=100;
% rnear=20;
xminit=xminit(1:rn,:);
 % xminit=xminit(randi(size(xminit,1),1,3),:)
 xminit(1,:)=[a sort(a+(b-a)*rand(1,M-1)) b];
% xminit(1,:)=linspace(a,b,M+1);
xminit(5,:)=[linspace(a,b-(b-a)/2,M) b];
xminit(6,:)=[a linspace(a+(b-a)/2,b,M)];
xminit(7,:)=[a linspace(a+(b-a)/3,b,M)];
xminit(8,:)=[linspace(a,b-(b-a)/3,M) b];
for n1=70:90
...
% p_b=1-(1/M)^r;
    ...
    for r=1:rn % loop over initializations
        ...
        save(strcat('uniform1_xcubedy_fmincon_M',num2str(M),'pb',num2str(p_b),'a',num2str(a),'b',num2str(b),'.mat'),'xm','ym','dist_enc','dist_dec','erm','xrm','yrm','drm','xminit','exitrn','p_b');
    end
    toc
    % for r=1:rn
    ...
    save(strcat('uniform1_xcubedy_fmincon_M',num2str(M),'pb',num2str(p_b),'a',num2str(a),'b',num2str(b),'.mat'),'xm','ym','dist_enc','dist_dec','erm','xrm','yrm','drm','xminit','exitrn','numlocopt','cmp','cmpx','indx','p_b','ctr');
...
der=0;
    ...
    if xm(i-1)~=xm(i)
        dyixi=c2*f1(xm(i))*(xm(i)-ym(i-1))/(c1+c2*integral(f1,xm(i-1),xm(i),'ArrayValued',true));
        der=der-2*c2*dyixi*integral(f3_1,xm(i-1),xm(i),'ArrayValued',true);
    end
    if xm(i)~=xm(i+1)
        dyi1xi=-c2*f1(xm(i))*(xm(i)-ym(i))/(c1+c2*integral(f1,xm(i),xm(i+1),'ArrayValued',true));
